package my.edu.umk.pams.account.financialaid;

import org.junit.runner.RunWith;
import org.junit.runners.Suite;

@RunWith(Suite.class)
@Suite.SuiteClasses({
    US_AC_FNA_1000.class, //hajar combine with US_AC_FNA_1009
    US_AC_FNA_1001.class, //hajar
    US_AC_FNA_1002.class, //hajar
    US_AC_FNA_1005.class,
    US_AC_FNA_1006.class,
    US_AC_FNA_1007.class,
    US_AC_FNA_1009.class,
    US_AC_FNA_2003.class,
    US_AC_FNA_3000.class, //hajar
    US_AC_FNA_4000.class,
})
public class FinancialAidModuleTestSuite {

}
