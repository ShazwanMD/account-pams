...
@Component({
  selector: 'pams-debit-note-receipt',
  templateUrl: './debit-note-receipt.dialog.html',
})
...
export class DebitNoteReceiptDialog implements OnInit {
  ...
  save(receipt: Receipt, isValid: boolean) {
    ...
    this.store.dispatch(this.actions.addReceiptDebitNote(this._receipt, this.createForm.get('debitNote').value));
    this.dialog.close();
  }
}
