...
export class AccountActivityListComponent {
    ...
    private editorDialogRef1: MdDialogRef<ReceiptItemDialog>
    private editorDialogRef4: MdDialogRef<DebitNoteItemDialog>
    private editorDialogRef5: MdDialogRef<CreditNoteItemDialog>
    ...
    showDialog( activity ): void {
        ...
        if ( activity.transactionCode === this.transaction[0] ) {
        ...
        else if ( activity.transactionCode === this.transaction[1] ) {
            this.editorDialogRef1 = this.dialog.open( ReceiptItemDialog, config );
            ...
            this.editorDialogRef4 = this.dialog.open( DebitNoteItemDialog, config );
            this.editorDialogRef4.componentInstance.activity = activity;
            this.editorDialogRef4.afterClosed().subscribe(( res ) => {
                console.log( 'close dialog' );
            } );
        }
        else if ( activity.transactionCode === this.transaction[5] ) {
            this.editorDialogRef5 = this.dialog.open( CreditNoteItemDialog, config );
            this.editorDialogRef5.componentInstance.activity = activity;
            this.editorDialogRef5.afterClosed().subscribe(( res ) => {
                console.log( 'close dialog' );
            } );
        }
